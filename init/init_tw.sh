...
readonly root_dir="$HOME/mongoDB_tw"
...
readonly db_file_name="db_init_tw.js"  # initialize the database
...
function create_container() {
	sudo docker run --name tw_spider --privileged --restart=always  \
  -p 27019:27019 \
  -v "$data_path":/data/db \
...
function main() {
	...
	cat <<-EOF
Run command
	"sudo docker exec -it wb_spider mongo 127.0.0.1:27019 /etc/resource/db_init_tw.js"
EOF
}
